...
def get_or_create_conversation(
    conversation_id: Optional[int], user_id: int, db: Session
) -> Conversation:
    """
...
def load_conversation_history(conversation_id: int, db: Session) -> list[dict[str, str]]:
    """
    ...
    if use_rag:
        try:
            ...
            if rag_context:
                ...
                logger.info("ℹ️  No RAG context (not programming-related or no matching docs)")
        except Exception as e:
            logger.warning(f"⚠️  RAG context retrieval failed: {e}")
    else:
        logger.info("⏭️  RAG skipped (toggle off)")
