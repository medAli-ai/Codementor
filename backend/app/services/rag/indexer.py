...
def _rects_overlap(r1: tuple, r2: tuple, margin: float = 2.0) -> bool:
    ...
    """
    return not (
        r1[2] < r2[0] - margin  # r1 is left of r2
        or r1[0] > r2[2] + margin  # r1 is right of r2
        or r1[3] < r2[1] - margin  # r1 is above r2
        or r1[1] > r2[3] + margin  # r1 is below r2
    )
...
class Indexer:
    ...
    def extract_pdf_elements(self, pdf_path: str) -> tuple[list[PDFElement], dict]:
        ...
        try:
            ...
            logger.info(
                f"✅ Extracted {len(all_elements)} elements from {metadata['pages_count']} pages"
            )
            logger.info(
                f"   Code blocks: {stats['code']}, Tables: {stats['table']}, Prose: {stats['prose']}"
            )
    ...
    def _extract_page_elements(self, page, page_num: int) -> list[PDFElement]:
        ...
        code_elements = self._extract_code_blocks(page_dict_blocks, page_num, claimed_regions)
        ...
        prose_elements = self._extract_prose(page_dict_blocks, page_num, claimed_regions)
        ...
        try:
            ...
            for table in tables:
                ...
                if table_text and len(table_text) > 10:
                    elements.append(
                        PDFElement(
                            type="table", text=table_text, page_num=page_num, bbox=table.bbox
                        )
                    )
            ...
            if mono_blocks:
                ...
                for group in groups:
                    ...
                    if text and len(text) > 5:
                        ...
                        elements.append(
                            PDFElement(
                                type="code", text=text, page_num=page_num, bbox=(x0, y0, x1, y1)
                            )
                        )
    ...
    def _chunk_elements(self, elements: list[PDFElement]) -> list[dict]:
        """
        ...
        if prose_elements:
            ...
            logger.info(
                f"   Prose chunks: {len(prose_chunks)} (semantic chunked from {len(prose_elements)} elements)"
            )
    ...
    def index_pdf(self, pdf_path: str, collection_name: str, metadata: dict) -> int:
        """
        ...
        try:
            ...
            logger.info("✂️  Chunking elements...")
            ...
            for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                ...
                point = PointStruct(
                    id=str(uuid.uuid4()), vector=embedding.tolist(), payload=point_metadata
                )
                points.append(point)
    ...
    def delete_document(self, collection_name: str, document_id: int) -> bool:
