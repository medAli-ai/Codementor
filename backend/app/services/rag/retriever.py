...
class Retriever:
    ...
    def _build_filter(
        self, user_id: Optional[int], include_public: bool, topic: Optional[str] = None
    ) -> Optional[Filter]:
        """
        ...
        if include_public:
            should_conditions.append(FieldCondition(key="is_public", match=MatchValue(value=True)))

        # Topic condition (AND logic)
        must_conditions = []
        if topic:
            must_conditions.append(FieldCondition(key="topic", match=MatchValue(value=topic)))
    ...
    def retrieve_chunk_preview(
        ...
        logger.info(f"Fetching chunk preview: doc={document_id} idx={chunk_index} window={window}")
